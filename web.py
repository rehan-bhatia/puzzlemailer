...
def load_counter():
    if os.path.exists(COUNTER_FILE):
        with open(COUNTER_FILE, "r") as f:
            return json.load(f)
    return {"used_ids": [], "used_titles": [], "alternator": 0}
...
def log_sent_email(subject, body):
    ...
    log_entry = {"timestamp": now, "subject": subject, "body": body}
...
def handle_send(mode):
    ...
    if mode == "question":
        ...
        if not os.path.exists(TODAY_FILE):
            raise Exception("No puzzles stored for today. Run /send-question first.")
